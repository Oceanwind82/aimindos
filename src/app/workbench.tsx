...
export default function Workbench() {
  ...
  return (
    <div
      className="relative h-[70vh] rounded-2xl border border-white/10 bg-gradient-to-br from-neutral-900 to-black overflow-hidden"
    ...
    >
      {panels.map((p) => (
        <div
          key={p.id}
          className={`absolute rounded-2xl border border-white/10 bg-black/30 shadow select-none panel-${p.id}`}
        >
          <div
            className="flex h-10 items-center justify-between border-b border-white/10 px-3"
...
function Logs() {
  ...
  return (
    <div className="space-y-2">
      <button
        className="inline-flex items-center justify-center rounded-xl px-4 py-2 border border-white/10 hover:bg-white/10"
        onClick={() => setLogs((l) => [...l, `Ping @ ${new Date().toLocaleTimeString()}`])}
      >
        Add Log
      </button>
